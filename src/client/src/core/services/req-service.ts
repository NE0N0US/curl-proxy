...
export class ReqService {
	...
	static extractQurlProxy(req: Req) {
		const
		...
		// flags
		req.options.qurlProxy.fastest = url.searchParams.has(SearchParam.FASTEST)
		req.options.qurlProxy.renResHeaders = url.searchParams.has(SearchParam.REN_RES_HEADERS)
		req.options.qurlProxy.skipDefaults = url.searchParams.has(SearchParam.SKIP_DEFAULTS)
		// primitives
		req.options.qurlProxy.retry = url.searchParams.get(SearchParam.RETRY) ?? ''
		req.options.qurlProxy.retryIn = url.searchParams.get(SearchParam.RETRY_IN) ?? ''
		req.options.qurlProxy.retryFactor = url.searchParams.get(SearchParam.RETRY_FACTOR) ?? ''
		req.options.qurlProxy.retryLimit = url.searchParams.get(SearchParam.RETRY_LIMIT) ?? ''
		req.options.qurlProxy.timeout = url.searchParams.get(SearchParam.TIMEOUT) ?? ''
		req.options.qurlProxy.ttfb = url.searchParams.get(SearchParam.TTFB) ?? ''
		req.options.qurlProxy.throttle = url.searchParams.get(SearchParam.THROTTLE) ?? ''
		req.options.qurlProxy.throttleUp = url.searchParams.get(SearchParam.THROTTLE_UP) ?? ''
		req.options.qurlProxy.status = url.searchParams.get(SearchParam.STATUS) ?? ''
		req.options.qurlProxy.method = url.searchParams.get(SearchParam.METHOD) ?? ''
		req.options.qurlProxy.resBody = url.searchParams.get(SearchParam.RES_BODY) ?? ''
		// objects
		...
		req.options.qurlProxy.headersAll.headers!.rows = headers!
		req.options.qurlProxy.headersAll.resHeaders!.rows = resHeaders!
		// arrays
		...
		req.options.qurlProxy.headersAll.delHeaders!.rows = delHeaders!
		req.options.qurlProxy.headersAll.delResHeaders!.rows = delResHeaders!
		// body
		req.options.qurlProxy.body.type =
			url.searchParams.has(SearchParam.BODY) ? ReqBodyType.TEXT : ReqBodyType.NONE
		req.options.qurlProxy.body.value = url.searchParams.get(SearchParam.BODY)
	}
	...
	static deserialize(obj: any) {
		...
		if (obj.extractQurlProxy)
			ReqService.extractQurlProxy(req)
	...
	static async serialize(req: Req) {
		...
		return {
			...
			extractQurlProxy:
				(!!req.options.qurlProxy.server.value &&
				!req.options.qurlProxy.server.disable) ? true : undefined,
		}
	}
}
