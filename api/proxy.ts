...
function formatHelp(message?: string, serviceUrl = 'http://localhost:3000/', html = false) {
	...
	return html ? helpHtml.replace('HELP_TEXT', escapeHtml(text)) : text
}
...
function deleteWildcard(record: Record<string, string>, key: string) {
...
function lowerKeys(record: Record<string, string>): typeof record {
...
async function proxy(req: Request) {
	...
	try {
		...
		const
			...
			timeout = Math.min(0, Number.isSafeInteger(+timeoutParam) ? +timeoutParam : 0),
		//make response
		...
		;[
			...
			?.forEach(name => deleteWildcard(headers, name))
